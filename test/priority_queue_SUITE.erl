...
init_per_group(parallel_tests, Config) ->
    rabbit_ct_helpers:run_steps(Config, [
        fun rabbit_ct_broker_helpers:prepare_connections/1
      ]);
...
end_per_group(parallel_tests, Config) ->
    rabbit_ct_helpers:run_steps(Config, [
        fun rabbit_ct_broker_helpers:terminate_connections/1
      ]);
...
recovery(Config) ->
    ...
    %% coverage.
    rabbit_ct_broker_helpers:restart_broker_i(Config, 0),
...
simple_order(Config) ->
    {ok, Ch} = rabbit_ct_broker_helpers:open_channel(Config, 0),
    ...
    get_all(Ch, Q, do_ack, [3, 3, 3, 2, 2, 2, 1, 1, 1]),
    delete(Ch, Q),
    rabbit_ct_broker_helpers:close_channel(Ch),
    passed.
...
mirror_queue_sync(Config) ->
    ...
    Nodename0 = rabbit_ct_broker_helpers:get_node_config(Config, 0,
      rmq_nodename),
    rabbit_ct_broker_helpers:control_action(sync_queue, Nodename0,
      [binary_to_list(Q)], [{"-p", "/"}]),
    wait_for_sync(Nodename0, rabbit_misc:r(<<"/">>, queue, Q)),
    passed.
...
wait_for_sync(Nodename, Q) ->
    case synced(Nodename, Q) of
        true  -> ok;
...
synced(Nodename, Q) ->
    Info = rabbit_ct_broker_helpers:run_on_broker(Nodename,
